...
def make_book(rm: ResourceManager, i18n: I18n, local_instance: bool = False, reverse_translate: bool = False):
    ...
    book.category('the_world', 'The World', 'All about the natural world around you.', 'tfc:grass/loam', is_sorted=True, entries=(
        ...
        entry('climate', 'Calendar and Climate', 'tfc:textures/gui/book/icons/thermometer.png', pages=(
            ...
            text('Temperature in TerraFirmaCraft is influenced by a number of factors:$(br)$(li)Firstly, the region, especially the latitude (Z coordinate) will play the largest role.$(li)Secondly, the current season will influence the temperature - it will be hottest during Summer, and coldest during Winter.$(li)Thirdly, the temperature can be different day to day as well as varying from hour to hour.').anchor('temperature'),
            text('$(li)Finally, elevation (Y coordinate) affects temperature. Above sea level, temperature drops by 1.6°C per 10 blocks. Below sea level, it trends toward the area\'s annual average.'),
        ...
        entry('wild_fruits', 'Wild Fruits', 'tfc:food/elderberry', pages=(
            ...
            text('Bushes, unlike fruit trees, take into account surrounding water blocks to determine their $(l:mechanics/hydration)Hydration$(), unlike fruit trees, which only care about rainfall.$(br)Any full bush block can grow berries, which are harvestable with $(item)$(k:key.use)$().$(br)If local temperature or hydration is outside the bush\'s listed range, it may become dormant. If it stays dormant for 4 consecutive months at a time when it should be active, the bush will die.'),
            page_break(),
            ...
            text('If local temperature or hydration is outside the bush\'s listed range (or special water needs are not met), it may become dormant. If it remains dormant for 4 consecutive months when it should be active, the bush will die.'),
